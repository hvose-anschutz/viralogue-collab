...
with open('SRR32782394_CodingCountTable.txt','r') as g:
    ...
    for items in g.readlines():
        ...
        if float(ERV[9]) > 1:
            #this index is the average or sample column, we only want ERVs included in the count if their average expression score across the experiment (or per sample) is above whatever float we indicate
